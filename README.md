...
При нажатии кнопки `Создать топологию` высовется метод `create` (описание работы ниже), который на стороне сервера опросит все устройства и создаст новую топологию в списке. При выборе новой созданной топологии в списке, отобразится ее графическое представление

При нажатии кнопки `Сравнение топологий` вызывается модельное окно сравнения двух топологий

![Окно сравнения топологий](/img/compare_screen.PNG?raw=true)

Необходимо выбрать две топологии для сравнения, при нажатии кнопки `Сравнить топологии` в окне сравнения, на стороне сервера будет вызван метод `copmare` в качестве параметров будут переданы имена файлов топологий для сравнения
...
1. Собирает информацию о LLDP соседях с каждого устройства из `inventory`

2. Строит на основании полученной инфомации топологию в формате JSON `{ nodes: [список узлов], edges: [список соединений] }`

3. Сохраниет полученную топологию в каталог topologies с именем `topology-DATE-TIME`
